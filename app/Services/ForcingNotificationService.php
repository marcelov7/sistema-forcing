...
class ForcingNotificationService
{
     ...
     * Para: Liberadores da mesma unidade + Admins
     */
    public function notificarForcingCriado(Forcing $forcing)
    {
        try {
            // Buscar liberadores da mesma unidade + admins
            $liberadores = User::where(function($query) use ($forcing) {
                    $query->where('perfil', 'liberador')
                          ->where('unit_id', $forcing->unit_id);
                })
                ->orWhere('perfil', 'admin')
                ->get();

            // Se não há liberadores na unidade, buscar todos os liberadores
            if ($liberadores->where('perfil', 'liberador')->count() === 0) {
                $liberadores = User::where('perfil', 'liberador')
                    ->orWhere('perfil', 'admin')
                    ->get();
            }

            // Enviar emails de forma assíncrona se possível, senão síncrona
            foreach ($liberadores as $liberador) {
                try {
                    // Tentar enviar de forma assíncrona primeiro
                    Mail::to($liberador->email)->queue(new ForcingCriado($forcing));
                } catch (\Exception $queueException) {
                    // Se falhar, enviar de forma síncrona
                    Log::warning("Queue não disponível, enviando email síncrono", [
            ...
            Log::info("Notificação de forcing criado enviada", [
                ...
                'unit_id' => $forcing->unit_id,
                'destinatarios' => $liberadores->pluck('email')->toArray(),
                'total_destinatarios' => $liberadores->count()
            ]);
